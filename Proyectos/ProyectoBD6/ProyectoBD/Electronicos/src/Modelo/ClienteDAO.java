...
public class ClienteDAO {
    ...
    // Función para buscar un cliente por su ID_CLIENTE
    public Cliente buscarClientePorID(String idCliente) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                cliente.setTELEFONO_C(rs.getString("TELEFONO_C"));
                cliente.setCORREO_C(rs.getString("CORREO_C"));
     ...
     public List MostrarCliente()
    {
        ...
        try 
        {
            ...
            while (rs.next())
            {
                ...
                cli.setTELEFONO_C(rs.getString("TELEFONO_C"));
                cli.setCORREO_C(rs.getString("CORREO_C"));
                MostrarCli.add(cli);
            }
        } 
        catch (SQLException e) 
    ...
    {
        String sql = "INSERT INTO CLIENTE (ID_CLIENTE, NOMBRE_CLIENTE, TELEFONO_C, CORREO_C) "
                   + "VALUES(?,?,?,?)";
        try 
        {
            con = on.getConnection();
            ...
            ps.setString(3, cli.getTELEFONO_C());
            ps.setString(4, cli.getCORREO_C());
            ps.execute();
            return true;
        } 
        catch (SQLException e) 
    ...
    {
        String sql = "UPDATE CLIENTE SET NOMBRE_CLIENTE = ?, TELEFONO_C = ?, CORREO_C = ?"
                    + "WHERE ID_CLIENTE = ?";
        try 
        {
            ps = con.prepareStatement(sql);
            ps.setString(1, cli.getNOMBRE_CLIENTE());
            ps.setString(2, cli.getTELEFONO_C());
            ps.setString(3, cli.getCORREO_C());
